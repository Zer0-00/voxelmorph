...
parser.add_argument('--model-dir', default='model_second',
...
parser.add_argument('--lr', type=float, default=1e-4, help='learning 1rate (default: 1e-4)')
...
writer = tf.summary.create_file_writer(logdir=os.path.join('.', 'logs'), max_queue=10)
class showAtlas(tf.keras.callbacks.Callback):
    def __init__(self):
        super(showAtlas, self).__init__()
            
    def on_epoch_end(self, epoch, logs=None):
        vxm.py.utils.save_volfile(self.model.get_atlas(), os.path.join(model_dir, 'template_{}.nii.gz'.format(epoch)))
...
with tf.device(device):
    ...
    with writer.as_default():
        model.fit_generator(generator,
                            ...
                            epochs=args.epochs,
                            callbacks=[save_callback,showAtlas(),tensorboard_callback],
